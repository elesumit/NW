...
Object.entries(config.viewPorts).forEach(([viewportName, viewportSetting]) => {
    ...
    describe(`Validate fund list page - ${viewportName}`, () => {
        beforeEach(() => {
            cy.visit(config.environments.baseURL + "/" + config.environments.fundList);
        ...
        describe('Validate fund list page', () => {
           
            it('Validate links on fund list page', () => { 
                let currentPageUrl;
                ...
                utils.readCsvrows("fundListPage.csv").then((rowsWithExecutionYes) => {
