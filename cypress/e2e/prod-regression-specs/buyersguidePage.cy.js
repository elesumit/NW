...
Object.entries(config.viewPorts).forEach(([viewportName, viewportSetting]) => {
    ...
    describe(`Validate buyersguide Page - ${viewportName}`, () => {
        beforeEach(() => {
            cy.visit(config.environments.baseURL + "/" + config.environments.buyerGuide);
        ...
        describe('Validate buyersguide Page', () => {
            it('Validate links on buyersguide Page', () => {
                let currentPageUrl;

                cy.url().then(url => {
                    currentPageUrl = url;
                });
                utils.readCsvrows("buyerGuide.csv").then((rowsWithExecutionYes) => {
